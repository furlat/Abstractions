"""
Example 2: Distributed Addressing (Async Version)
...
- Async execution with proper event handling
"""
...
async def update_gpa_async(student: Student, new_gpa: float) -> Student:
    # Simulate async work (like database update)
    await asyncio.sleep(0.001)
    student.gpa = new_gpa
    return student

@CallableRegistry.register("create_transcript_async")
async def create_transcript_async(name: str, gpa: float, courses: List[str]) -> Course:
...
@CallableRegistry.register("create_transcript_sync")
def create_transcript_sync(name: str, gpa: float, courses: List[str]) -> Course:
    """Create a transcript-like entity from distributed data."""
    return Course(name=f"Transcript for {name}", credits=len(courses), grade=f"GPA: {gpa}")

async def run_tests_async() -> Tuple[int, int, List[str], List[str]]:
    """Run async tests and validate distributed addressing claims."""
    ...
    print("=== Testing Distributed Addressing (Async) ===\n")
    ...
    # Test 4: Async location transparency
    async def test_async_location_transparency():
        # Functions can use addressed data without knowing location
        transcript = await CallableRegistry.aexecute("create_transcript_async",
    ...
    # Run async test
    try:
        await test_async_location_transparency()
        test_feature("Async location transparency", lambda: True)
    except Exception as e:
        test_feature("Async location transparency", lambda: exec('raise Exception(str(e))'))
    
    # Test 5: Mixed local and remote data (async)
    async def test_async_mixed_data():
        # Test mixing addressed data with direct values
        courses = ["Biology", "Chemistry"]
        transcript2 = await CallableRegistry.aexecute("create_transcript_async",
    ...
    # Run async test
    try:
        await test_async_mixed_data()
        test_feature("Async mixed local and remote data", lambda: True)
    except Exception as e:
        test_feature("Async mixed local and remote data", lambda: exec('raise Exception(str(e))'))
    
    # Test 6: Sync vs Async comparison
    async def test_sync_vs_async_comparison():
        # Test that sync functions work via aexecute too
        transcript_sync = await CallableRegistry.aexecute("create_transcript_sync",
            name=f"@{updated.ecs_id}.name",
            gpa=f"@{updated.ecs_id}.gpa",
            courses=["History", "Literature"]
        )
        
        transcript_sync_result = transcript_sync if not isinstance(transcript_sync, list) else transcript_sync[0]
        assert isinstance(transcript_sync_result, Course)
        assert "Alice" in transcript_sync_result.name
        assert transcript_sync_result.credits == 2
    ...
    try:
        ...
        test_feature("Sync vs Async comparison", lambda: exec('raise Exception(str(e))'))
    
    # Test 7: Address resolution error handling
    ...
    # Test 8: Entity identity preservation
    ...
    # Test 9: Distributed system readiness
    ...
    # Test 10: Async function metadata
    ...
    test_feature("Async function metadata", test_async_metadata)
...
async def main():
    ...
    print("🌐 Testing distributed addressing capabilities (async)...")
    ...
    print(f"\n✅ Async distributed addressing example completed!")
    
    # Show the benefits
    print(f"\n📊 Key Benefits of Async Distributed Addressing:")
    ...
    print(f"  - 🚀 Async ready - works with async functions and event loops")
    print(f"  - 🔄 Proper event emission - full observability in async context")
