# Language Modelling Agents are Policies in a Program Space
...
I believe this has mostly to do with our lack of understanding, or better, a lack of framing for what LLM agents actually are. As a result, many doubts arise about what they should do and how they should do it. This is not surprising, given the wide range of theories that tempt us while navigating such a complex problem. Moreover, agency is only half of the story: if they are agents, what is their environment?

We will focus on the general setup of a function-calling agent equipped with a finite set of functions, a computing engine to execute them, and a terminal reward signal. Think of one of the default LangChain or PydanticAI agents interacting with a Python REPL and receiving a feedback from the user at program termination. Informally it is quite easy to interpret what is happening in the successful case: the user inputs an objective and the algorithm implements a program to achieve it. The goal of this post is to contribute a formal vision for this ubiquitous setup that lifts the autoregression of symbolic sequences into a coherent theory of agentic program synthesis.

First, we want to properly characterize the interaction between the agent and its computing engine as a Partially Observable Markov Decision Process. This will require a clear separation between the environment’s state, its afferents, the resulting state update rules, and how these elements appear as the agent’s observations and actions. This step will give us a target for the abstractions that must be derived from the underlying transformer model for a proper POMDP to emerge. 
