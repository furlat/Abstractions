...
We define a process composed of a set of possible environment states $\mathcal{S}$, actions $\mathcal{A}$ available to the agent, and observations $\mathcal{O}$ available to the policy.  
The evolution of the system is governed by the following stochastic kernels:

- $P(s' \mid s, a)$, the **transition kernel**, describing how the environment changes after an action;  
- $\Omega(o \mid s', a)$, the **observation kernel**, specifying what part of the new state is visible to the agent;  
- $\pi(a \mid o)$, the **policy**, describing how the agent selects its next action based on the current observation;  
- and $R(s, a)$, the **reward function**, which evaluates the desirability of transitions.
...
$$
\pi(a \mid o, g), \quad R(s, a, g)
$$

This view is particularly useful for our purpose, as most function-calling agents operate under an explicit task or user request that naturally acts as a goal condition.
...
Each conditional distribution $P_\theta(\text{turn}_t \mid \text{turn}_{<t})$ represents the **behavioral policy** of the agent at the turn level.  
...
Given the token-level probability $P_\theta(\tau_t \mid \tau_{<t})$, the probability of generating a specific subsequence of length $L$ is:

$$
P_\theta(\tau_{t:t+L}) = \prod_{k=0}^{L-1} P_\theta(\tau_{t+k} \mid \tau_{<t+k})
$$
...
$$
(f_t, x_t): \quad x_t \mapsto y_t = f_t(x_t), \qquad x_t \in \{ y_i \mid i < t \} \cup s_0.
$$
...
Given this structure, the agent’s **behavioral policy** defines the probability of selecting the next valid function and its corresponding input, conditioned on the visible history and the initial environment state:
...
This is the turn-level analogue of the policy $\pi(a_t \mid o_t)$ in the general POMDP formulation.
...
In the general case, the environment is **stateful**. Functions may have **non-degenerate side effects**, meaning that while their input–output mapping remains constant, they can modify the internal state of the system.  
...
| **Observation** $o_t$ | The visible context to the LLM, composed of text, tool responses, and structured data appended to the prompt |
| **Action** $a_t$ | The generation of a complete turn: a structured function call or reasoning segment produced by the LLM |
| **Policy** $\pi(a_t \mid o_t)$ | The LLM’s autoregressive generation process constrained by the environment’s valid-action support |
| **Transition** $P(s_{t+1} \mid s_t, a_t)$ | The update of the environment’s internal state following a function execution |
| **Observation kernel** $\Omega(o_{t+1} \mid s_{t+1}, a_t)$ | The process by which the environment produces the next visible context (e.g., tool output) |
...
\sum_L
\left(
\prod_{k=0}^{L-1} P_\theta(\tau_{k_t+k} \mid \tau_{<k_t+k})
\right)
...
which is appended to the ongoing context forming the next observation:

$$
o_{t+1} = ((y_i, (f_i, x_i)))_{i \leq t}
$$
...
Equivalently, $\text{Stack}_t$ is a **sufficient statistic** of the history and the history is a sufficient statistics of the memory state.  
