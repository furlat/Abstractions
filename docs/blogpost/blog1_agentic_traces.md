...
The following table enumerates all possible observation histories of length at most one (one action and one observation), their associated beliefs, the entropy of the belief $H(b_t) = -b_t \log_2 b_t - (1-b_t) \log_2 (1-b_t)$ measuring the agent's uncertainty, the expected reward for each action, and the predictive probabilities of blessing ($o_{t+1} = 0$) conditional on each action. Let $R(\text{blessing}) = +1$ and $R(\text{curse}) = -1$.
...
The table makes visible that all the two-token suffixes partition into exactly three equivalence classes based on their predictive content. The first class contains only the empty history at epoch start with maximum uncertainty. The second class contains the two turns where act equals omen, $(0,0)$ and $(1,1)$, corresponding to the loyal god now guarding with zero uncertainty. The third class contains the two turns where act differs from omen, $(0,1)$ and $(1,0)$, corresponding to the trickster with zero uncertainty. The last two columns show the predictive state representation: the probability of blessing under each action. These predictive probabilities cluster into the same three classes as the beliefs, $(1/2, 1/2)$, $(1, 0)$, and $(0, 1)$, which is no coincidence. In a synchronizable environment the belief process and the predictive state process are informationally equivalent.

Once synchronized the optimal policy is trivial: always offer when $b = 1$ (loyal), always nothing when $b = 0$ (trickster). At $b = 1/2$ both actions have zero expected reward, so the first turn is a coin flip that reveals the phase. The disciple studying the pythia's notebook has enough information to recover these equivalence classes and, from them, the optimal policy. With enough data the theoretical emission probabilities described in the table can be empirically computed for all histories up to length 100, leading to a highly redundant matrix where all histories with the same terminal joint prefix have the same next token probability.

We have seen how knowledge of the generator model allows to develop an optimal belief based policy that converges to the true generator states after the first action, and how the empirical transitions from histories can be used to estimate the generator model by clustering sequences with the same conditional futures. We now ask ourselves if we can simply train a powerful autoregressive predictor of the action conditioned next token given the joint history $z_{1:t}$ and use its hidden state as a sufficient statistic for the generator state. The theoretical discussion in the previous section presents some formal guarantess that any unifilar sufficient statistic of the observable process of a synchronizable environment must have a contracting function that maps to the generator state. In practice, the sufficiency-minimality relationship between sufficient states and causal states imply that if we can guarantee that a trained transformer is a sufficient statistic for the observable process's future for all possible actions then we can also learn a probe from its hidden state to the generator states, hence the learned states are sufficient for optimal control. 

Concretely, we implement the shrine dynamics above as a 100-step episodic environment and feed the agent only a short window of its own interaction history. In the experiments below we use a history of actions and observations of length $L$, so the network input at time (t) is the last $L$ joint symbols $(a_t,o_t)$ encoded as a length-2 binary vector. A small transformer encoder treats this two-symbol vector as a token sequence, applies token and positional embeddings, and produces a hidden state $(h_t)$ from the final token. The Q-head reads $h_t$ to select the next action $a_{t+1}$, so decisions are always made from prefixes that end in an observation (omen). In parallel, an auxiliary head is trained to predict the next the next joint $(a_{t+1},o_{t+1})$ in the same encoding from $h_t$ and the chosen action encoded as one-hot. Since the action component of the next observation is already provided as input, this auxiliary task effectively forces the transformer to approximate the conditional kernel $P(o_{t+1}\mid a_{t+1}, z_{\le t})$ while trivially copying the known action bit.  

We explore three training regimes: 1) pretrain the world model on random actions traces then freeze it and continue training the policy head, 2) train the world model and policy head together from the beginning, or 3) only train via the policy head. For all setups we also fit simple probes from $h_t$ to the current mode $s_t$, the next mode $s_{t+1}$, and the next latent $h_{t+1}$, which will let us to respectively measure the degree of sufficiency and unifilarity of the learned process. 
