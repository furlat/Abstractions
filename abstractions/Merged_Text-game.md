...
- Created: `2024-03-29T16:19:22.594191`
...
class GridMapWidget(Widget):
    ...
    def update(self, camera_control: CameraControl, player_position: Tuple[int, int]):
        ...
        print("fov: ", camera_control.toggle_fov, self.show_fog_of_war)  # Add this line
    ...
    def draw(self, surface: pygame.Surface, path: Optional[Path] = None, shadow: Optional[Shadow] = None,
        ...
        print("before the draw", self.show_fog_of_war)
        if fog_of_war and self.show_fog_of_war:
            print("using fog of war")
            visible_nodes = [node.position.value for node in fog_of_war.nodes]
        else:
            visible_nodes = self.grid_map_visual.node_visuals.keys()

        for position in visible_nodes:
