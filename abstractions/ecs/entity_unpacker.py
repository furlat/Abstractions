...
from abstractions.ecs.return_type_analyzer import ReturnAnalysis, ReturnPattern, ReturnTypeAnalyzer
...
class ContainerReconstructor:
    ...
    def unpack_with_signature_analysis(
        ...
        """
        Enhanced unpacking method that combines signature-time analysis with runtime result.
        
        This method bridges the gap between the Phase 2 signature analysis and the
        runtime unpacking process, enabling the callable registry to use both.
        
        Args:
            result: The actual function return value
            return_analysis_metadata: Signature-time analysis from ReturnTypeAnalyzer.analyze_type_signature()
            output_entity_class: The generated output entity class (may be used for wrapping)
        ...
        # Step 1: Perform runtime analysis on the actual result
        runtime_analysis = ReturnTypeAnalyzer.analyze_return(result)
        
        # Step 2: Combine signature analysis with runtime analysis
        combined_metadata = {
            **return_analysis_metadata,  # Signature-time analysis
            **runtime_analysis.container_metadata,  # Runtime container metadata
            "execution_id": str(execution_id),
    ...
    def _handle_sequence_unpack_enhanced(
        ...
        sequence_metadata = {
            **metadata,
            ...
            "entity_positions": list(range(len(entities)))
        }
    ...
    def _handle_mixed_unpack_enhanced(
        ...
        # Create container entity for non-entity data if needed
        container_entity = None
        if non_entity_data:
            container_entity = EntityUnpacker._create_container_entity(non_entity_data, {}, execution_id)
        
        mixed_metadata = {
            **metadata,
            "unpacking_type": "mixed",
            "entity_count": len(entities),
            "non_entity_data_present": bool(non_entity_data),
            "has_container_entity": container_entity is not None
        }
    ...
    def _handle_nested_unpack_enhanced(
        ...
        depth = EntityUnpacker._calculate_structure_depth(result)
        ...
        nested_metadata = {
            **metadata,
            ...
            "complexity_score": len(entities) * depth
        }
    ...
    def _handle_non_entity_enhanced(
        ...
        if output_entity_class:
            ...
            try:
                if hasattr(output_entity_class, 'model_fields') and 'result' in output_entity_class.model_fields:
                    # Output class expects 'result' field
                    wrapper_entity = output_entity_class(result=result)
                else:
                    # Try to create entity with result as direct value
                    wrapper_entity = output_entity_class(wrapped_value=result)
            except Exception:
                # Fallback to generic wrapper
                wrapper_entity = EntityUnpacker._create_wrapper_entity(result, execution_id)
        else:
            # Create generic wrapper entity
            wrapper_entity = EntityUnpacker._create_wrapper_entity(result, execution_id)
        ...
        non_entity_metadata = {
            **metadata,
            "unpacking_type": "wrapped_non_entity",
            "original_type": str(type(result).__name__),
            "used_output_entity_class": output_entity_class is not None,
            "wrapper_entity_type": type(wrapper_entity).__name__
        }
        
        return UnpackingResult(
            primary_entities=[wrapper_entity],
            container_entity=None,
            metadata=non_entity_metadata
        )
    
    @classmethod
    def _handle_fallback_wrapping(
        cls, 
        result: Any, 
        metadata: Dict[str, Any], 
        output_entity_class: Optional[type],
        ...
        """Fallback wrapping for unknown patterns."""
        wrapper_entity = EntityUnpacker._create_wrapper_entity(result, execution_id)
        
        fallback_metadata = {
            **metadata,
            "unpacking_type": "fallback_wrapped",
            "original_type": str(type(result).__name__),
            "fallback_reason": "unknown_pattern"
        }
